...
4. **Prefer OA when available.** Check OpenAlex, Unpaywall, and Europe PMC for open-access copies before attempting subscription routes.
...
- Transient network failures → retry the same route.
- Authorization / entitlement failures (403) → immediately try the next fallback route.

...
describe("Full Functional Integration Pipeline", () => {
  ...
  liveIt("should successfully search papers across multiple sources", async () => {
    const searchArgs = {
  ...
  liveIt("should successfully add a paper to a corpus", async () => {
  ...
  liveIt("should list all corpora", async () => {
  ...
  liveIt("should list papers in a specific corpus", async () => {
    ...
    expect(corpusData.papers.some((paper: any) => paper.title === testPaper.title)).toBeTruthy();
  });
});
